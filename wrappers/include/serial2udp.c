...
int serial2udp_add(int *remote_socket, int *remote_id, int *remote_if, char *serial2udp_map) {
    ...
    char tmp_dstport[10];
    memset((char *) &tmp_dstport, 0, sizeof(tmp_dstport));
    int rc = -1;
    ...
    for (++i; serial2udp_map[i] != ',' && serial2udp_map[i] != 0; i++) {
        ...
        strncat(tmp_dstport, &serial2udp_map[i], 1);
    }

    UNLLog(LLINFO, "Creating UDP MAP (src: %s, dst:%s:%s, dst_id %s:%s).\n", tmp_srcif, tmp_dst, tmp_dstport, tmp_dstid, tmp_dstif);
