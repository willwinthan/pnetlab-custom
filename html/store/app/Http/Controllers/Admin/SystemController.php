...
class SystemController extends Controller
{
    ...
    public function get(){
        ...
        $returnData = [
            ...
            CTRL_LOGO => Ctrl::get(CTRL_LOGO, '/store/public/assets/auth/img/logo.png'),
            CTRL_PRIMARY_COLOR => Ctrl::get(CTRL_PRIMARY_COLOR, '#0d274d'),
            CTRL_SECONDARY_COLOR => Ctrl::get(CTRL_SECONDARY_COLOR, '#1b4079'),
            CTRL_BG_COLOR => Ctrl::get(CTRL_BG_COLOR, '#0d274d'),
        ];
    ...
    public function uploadLogo(Request $request){
        ...
        if ($request->hasFile('logo')) {
            ...
            if (!in_array(strtolower($extension), ['png', 'jpg', 'jpeg', 'svg'])) {
                Reply::finish(false, 'ERROR', 'Invalid file type. Only PNG, JPG, JPEG, and SVG are allowed.');
            }
            $filename = 'custom_logo_' . time() . '.' . $extension;
            $destinationPath = public_path('assets/auth/img');
            $file->move($destinationPath, $filename);
            $logoPath = '/store/public/assets/auth/img/' . $filename;
            Ctrl::set(CTRL_LOGO, $logoPath);
            Reply::finish(true, 'success', ['logo_path' => $logoPath]);
        }
        Reply::finish(false, 'ERROR', 'No file uploaded.');
    }
