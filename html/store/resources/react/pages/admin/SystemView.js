...
class SystemView extends Component {
		...
		return (
			<>
				<div className='box_padding container'>
					...
					<div className='box_shadow box_padding'>
						<strong>{lang('Institution Branding Settings')}</strong>
						<hr></hr>
						<div className='row'>
							<div className='col-md-6' style={{marginBottom: 20}}>
								<div style={{fontWeight:'bold', marginBottom:10 }}>{lang('Institute Logo')}</div>
								<div style={{display: 'flex', alignItems: 'center'}}>
									<img src={this.state[CTRL_LOGO] || '/store/public/assets/auth/img/logo.png'} style={{maxHeight: 60, marginRight: 20, border: '1px solid #ccc', padding: 5, borderRadius: 4}} />
									<div>
										<input type='file' accept='image/*' onChange={(e) => this.handleLogoUpload(e)} style={{display: 'none'}} id='logo-upload-input' />
										<label htmlFor='logo-upload-input' className='button btn btn-primary' style={{cursor: 'pointer', margin: 0}}>{lang('Upload New Logo')}</label>
									</div>
								</div>
							</div>
							
							<div className='col-md-6' style={{marginBottom: 20}}>
								<div style={{fontWeight:'bold', marginBottom:10 }}>{lang('Colors Settings')}</div>
								<div style={{display: 'flex', justifyContent: 'space-between', gap: '15px', flexWrap: 'wrap'}}>
									<div className='box_flex' style={{flexDirection: 'column', alignItems: 'flex-start'}}>
										<label style={{fontSize: '11px', color: 'gray'}}>{lang('Primary Color')}</label>
										<div style={{display: 'flex', alignItems: 'center', gap: '5px'}}>
											<input type='color' value={this.state[CTRL_PRIMARY_COLOR] || '#0d274d'} onChange={(e) => this.setState({[CTRL_PRIMARY_COLOR]: e.target.value})} style={{width: 40, height: 30, border: 'none', cursor: 'pointer', marginRight: 5}} />
											<input className='input' type='text' value={this.state[CTRL_PRIMARY_COLOR] || '#0d274d'} onChange={(e) => this.setState({[CTRL_PRIMARY_COLOR]: e.target.value})} style={{width: 80, padding: '2px 5px'}} />
										</div>
									</div>
									<div className='box_flex' style={{flexDirection: 'column', alignItems: 'flex-start'}}>
										<label style={{fontSize: '11px', color: 'gray'}}>{lang('Secondary Color')}</label>
										<div style={{display: 'flex', alignItems: 'center', gap: '5px'}}>
											<input type='color' value={this.state[CTRL_SECONDARY_COLOR] || '#1b4079'} onChange={(e) => this.setState({[CTRL_SECONDARY_COLOR]: e.target.value})} style={{width: 40, height: 30, border: 'none', cursor: 'pointer', marginRight: 5}} />
											<input className='input' type='text' value={this.state[CTRL_SECONDARY_COLOR] || '#1b4079'} onChange={(e) => this.setState({[CTRL_SECONDARY_COLOR]: e.target.value})} style={{width: 80, padding: '2px 5px'}} />
										</div>
									</div>
									<div className='box_flex' style={{flexDirection: 'column', alignItems: 'flex-start'}}>
										<label style={{fontSize: '11px', color: 'gray'}}>{lang('Loading Screen Background')}</label>
										<div style={{display: 'flex', alignItems: 'center', gap: '5px'}}>
											<input type='color' value={this.state[CTRL_BG_COLOR] || '#0d274d'} onChange={(e) => this.setState({[CTRL_BG_COLOR]: e.target.value})} style={{width: 40, height: 30, border: 'none', cursor: 'pointer', marginRight: 5}} />
											<input className='input' type='text' value={this.state[CTRL_BG_COLOR] || '#0d274d'} onChange={(e) => this.setState({[CTRL_BG_COLOR]: e.target.value})} style={{width: 80, padding: '2px 5px'}} />
										</div>
									</div>
								</div>
							</div>
						...
						<div style={{display: 'flex', gap: '10px'}}>
							<button className='button btn btn-primary' onClick={() => this.saveBrandingSettings()}>{lang('Save Branding')}</button>
							<button className='button btn btn-default' onClick={() => this.resetBrandingSettings()} style={{border: '1px solid #ccc', marginLeft: 10}}>{lang('Reset to Defaults')}</button>
						</div>
					</div>
		...
		return axios.request({
			...
			.then(response => {
				...
				this.setState({
					...
					[CTRL_LOGO]: get(data[CTRL_LOGO], '/store/public/assets/auth/img/logo.png'),
					[CTRL_PRIMARY_COLOR]: get(data[CTRL_PRIMARY_COLOR], '#0d274d'),
					[CTRL_SECONDARY_COLOR]: get(data[CTRL_SECONDARY_COLOR], '#1b4079'),
					[CTRL_BG_COLOR]: get(data[CTRL_BG_COLOR], '#0d274d'),
				});
			})
	...
	handleLogoUpload(event) {
		...
		App.loading(true);
		axios.post('/store/public/admin/system/uploadLogo', formData, {
			headers: {
		...
		.then(response => {
			...
			if (res.result) {
				this.setState({ [CTRL_LOGO]: res.data.logo_path });
				showLog('Uploaded logo successfully. Please refresh pages to see full changes.', 'success');
			} else {
				error_handle(res);
			}
		})
	...
	saveBrandingSettings() {
		this.updateData({
			[CTRL_PRIMARY_COLOR]: this.state[CTRL_PRIMARY_COLOR],
			[CTRL_SECONDARY_COLOR]: this.state[CTRL_SECONDARY_COLOR],
			[CTRL_BG_COLOR]: this.state[CTRL_BG_COLOR],
		}).then(() => {
			showLog('Saved color configurations successfully.', 'success');
			window.location.reload();
		});
	}

	resetBrandingSettings() {
		makeQuestion('Do you want to reset branding to default colors and logo?', 'Yes', 'No').then(res => {
			if (!res) return;
			this.updateData({
				[CTRL_LOGO]: '/store/public/assets/auth/img/logo.png',
				[CTRL_PRIMARY_COLOR]: '#0d274d',
				[CTRL_SECONDARY_COLOR]: '#1b4079',
				[CTRL_BG_COLOR]: '#0d274d',
			}).then(() => {
				showLog('Branding reset successfully.', 'success');
				window.location.reload();
			});
		});
	}
